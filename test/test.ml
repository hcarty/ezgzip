let basic_predefined_checks () =
  ...
  Alcotest.(check (result string gzip_error))
    ...
    (Ezgzip.compress "hello world" |> Ezgzip.uncompress);
  Alcotest.(check (result string gzip_error))
    "Known bad"
    (Error (`Gzip (Invalid_content "placeholder message")))
    (Ezgzip.uncompress "probably not gzip");
  let big = String.make 1_000_000 'x' in
  Alcotest.(check (result string gzip_error))
    "Big-ish" (Ok big)
    (Ezgzip.compress big |> Ezgzip.uncompress);
  ()

let round_trip s =
  let compressed = Ezgzip.compress s in
  match Ezgzip.uncompress compressed with
  | Error err ->
...
let quickcheck_tests =
  [
    ("round-trip", `Quick, round_trip_random_cases "round-trip quickcheck" 1_000);
  ]
